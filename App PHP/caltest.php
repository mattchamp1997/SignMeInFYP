<?php
	...
	if($row1[0] == 'Student')
	{
		$sql2 = "SELECT moduleID from student_classlist WHERE studentID = '$idNum'";
		$result2 = mysqli_query($con, $sql2);
		
		while($row = mysqli_fetch_array($result2))
		{
			$sql3 = "SELECT * FROM class_details JOIN module_details ON class_details.moduleID = module_details.moduleID WHERE moduleID = '$row'";
			...
			while($row2 = mysqli_fetch_array($result3))
			{
				...
				"lateHr"=>$row2[11],"lateMin"=>$row2[12],"year"=>$row2[13],"month"=>$row2[14],"day"=>$row2[15],"moduleName"=>$row2[16]));
			}
		}
